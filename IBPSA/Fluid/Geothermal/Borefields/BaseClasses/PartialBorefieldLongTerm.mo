...
partial model PartialBorefieldLongTerm
  ...
  HeatTransfer.LoadAggregation.GroundTemperatureResponseLongTerm_ContinuousRecord
    groTemRes(
    ...
    gFuncStandard=gFuncStandard) "Ground temperature response"
  ...
  Modelica.Blocks.Interfaces.RealOutput deltaT(
    ...
    displayUnit="degC",
    start=0)
    "temperature difference between the  g-function and the cyclic one"
    annotation (Placement(transformation(extent={{100,58},{120,78}})));
  parameter Data.GFunctions.Template gFuncStandard
    annotation (Placement(transformation(extent={{0,-80},{20,-60}})));
  Modelica.Blocks.Interfaces.RealInput[nPred] QBor_flow_LT annotation (Placement(
        transformation(extent={{-120,4},{-80,44}}), iconTransformation(extent={
            {-120,4},{-80,44}})));
  parameter Integer nPred=15 "Number of considered long-term predictions";
  parameter Modelica.SIunits.Time tStep=604800
    "Time-step of the long-term predictions";
  parameter Real intervals[nPred + 1]={1,2,3,4,8,12,16,20,24,28,32,36,40,44,48,52}
    "Array with the long-term intervals to be evaluated, being the prediction period i = tStep*(interval[i+1]-interval[i])";
  Modelica.Blocks.Interfaces.RealOutput[nPred] delTBor_LT(
    final quantity="ThermodynamicTemperature",
    final unit="K",
    displayUnit="degC",
    each start=0)
