...
function temperatureResponseMatrixSoil
...
algorithm
  ...
  if forceGFunCalc or not Modelica.Utilities.Files.exist(pathSave) then
    (tGFun,gFun) :=
      IBPSA.Fluid.Geothermal.Borefields.BaseClasses.HeatTransfer.ThermalResponseFactors.gFunctionSoil(
      hBor=hBor,
      dBor=dBor,
      rBor=rBor,
      r=r,
...
end temperatureResponseMatrixSoil;
