...
model GroundTemperatureResponseLongTerm_ContinuousRecord
  ...
  parameter Integer nPred = 15
  "Number of considered long-term predictions";
  parameter Modelica.SIunits.Time tStep = 604800
  "Time-step of the long-term predictions";
  parameter Real[nPred+1] intervals = {1,2,3,4,8,12,16,20,24,28,32,36,40,44,48,52}
  "Array with the long-term intervals to be evaluated, being the prediction period i = tStep*(interval[i+1]-interval[i])";
  Modelica.Blocks.Interfaces.RealOutput[nPred] delTBor_LT(unit="K")
...
//  Modelica.SIunits.Time dummyTime = time^1;
...
//  "weighting function to take only the last value of optimization";
  Modelica.Blocks.Interfaces.RealInput[nPred] QBor_flow_LT(unit="W")
...
end GroundTemperatureResponseLongTerm_ContinuousRecord;
