...
partial model PartialBorefieldLin2
  ...
  replaceable HeatTransfer.GroundTemperatureResponse_Continuous                                    groTemRes(
...
protected
  ...
  Modelica.Blocks.Math.Add TSoiDis(each final k1=1, each final k2=1)
...
equation
  ...
  connect(groTemRes.delTBor, TSoiDis.u1) annotation (Line(points={{41,80},{60,
          80},{60,56},{-10,56},{-10,36},{8,36}}, color={0,0,127}));
  connect(repDelTBor.u, TSoiDis.y) annotation (Line(points={{36,30},{34,30},{34,
          30},{31,30}}, color={0,0,127}));
  connect(prescribedTemperature.T, repDelTBor.y)
    annotation (Line(points={{50,0},{70,0},{70,30},{59,30}}, color={0,0,127}));
  connect(prescribedTemperature.port, QBorHol.port_b)
    annotation (Line(points={{28,0},{0,0}}, color={191,0,0}));
  annotation (
